...
async function loadExercises(url = API) {
    ...
    //Delete button (JavaScript Generated)
    exercises.forEach(exercise => {
        ...
        <button onclick="editExercise('${exercise.id}', '${exercise.name}', '${exercise.type}', '${exercise.duration}')">Edit</button>
        </div>
        `;
    });
}
...
async function addExercise() {
    ...
    //Edit existing Exercise name, type and duration
async function editExercise(id, currentName, currentType, currentDuration) {
        const newName = prompt("Edit Exercise Name:", currentName);
        if (newName === null) return;

        let newType = prompt("Edit Exercise Type:", currentType);
    if (newType === null) return;

    let newDuration = prompt("Edit Exercise Duration:", currentDuration);
